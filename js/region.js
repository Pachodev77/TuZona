...
const displayRegionAds = () => {
    ...
    if (region) {
        ...
        const filteredAds = sampleAds.filter(ad => ad.location.toLowerCase().includes(region.toLowerCase()));

        regionAdsContainer.innerHTML = '';
        if (filteredAds.length > 0) {
            filteredAds.forEach(ad => {
                regionAdsContainer.innerHTML += createAdCard(ad);
            });
        } else {
